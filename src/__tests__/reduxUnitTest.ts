...
import { login, logout, loadDummyUsers, loadPosts, updatePosts, updateHeadline, updateProfile } from '../store/actions'
...
test('login the first user', async () => {

	let initState = persistedReducer(undefined, loadDummyUsers(users))
	let loginState = persistedReducer(initState, login(0));

	expect(loginState).toEqual({
		users: dummyUsersToUsers([], users),
		loadDummies: true,
		posts: [],
		loginState: {
			isLogin: true,
...
test('login an invalid user', () => {
	let initState = persistedReducer(undefined, loadDummyUsers(users))
	let loginState = persistedReducer(initState, login(10));
...
test('logout an user', () => {
	let initState = persistedReducer(undefined, loadDummyUsers(users))
	let loginState = persistedReducer(initState, login(1));
	let logoutState = persistedReducer(loginState, logout());
...
test("add articles when adding a follower", async () => {
	...
	userFriends.push(5);
	fetchPostState = persistedReducer(fetchPostState, updatePosts(userFriends, ""));
	expect(fetchPostState.displayPosts.length).toEqual(50);
});


test("add articles when removing a follower", async () => {
	mockedAxios.get.mockResolvedValueOnce({ data: posts });
	let response = await fetchPosts();
	let userIdx = 0;
	let userFriends = [2, 3, 4];
...
test("update the logged in user's headline", () => {
	let userIdx = 0;
	let newHeadline = "I want to be a youtuber!!!";

	let initState = persistedReducer(undefined, loadDummyUsers(users))
	let loginState = persistedReducer(initState, login(userIdx));
	loginState = persistedReducer(loginState, updateHeadline(newHeadline));

	expect(loginState.users[userIdx].headline).toEqual(newHeadline);
})


test("update the logged in user's profile", () => {
	let userIdx = 0;

	let initState = persistedReducer(undefined, loadDummyUsers(users))
	let loginState = persistedReducer(initState, login(userIdx));

	let newProfile: User = { ...loginState.users[userIdx] };
	let newEmail = "hw55@stanford.gov";
	let newName = "Micheal Jackson";

	newProfile.email = "hw55@stanford.gov";
	newProfile.name = "Micheal Jackson";

	loginState = persistedReducer(loginState, updateProfile(newProfile));


	expect(loginState.users[userIdx].email).toEqual(newEmail);
	expect(loginState.users[userIdx].name).toEqual(newName);
