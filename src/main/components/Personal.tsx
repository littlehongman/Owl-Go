...
const Personal = ({username, userAvatar}: UserProps) => {
    const [headline, setHeadline] = useState<string>("Experienced developer seeking for internship opportunity")
    const inputRef = useRef<HTMLTextAreaElement>(null);

    const handleClick = () => {
        if (inputRef.current?.value !== undefined && inputRef.current?.value !== ""){
            setHeadline(inputRef.current?.value);
            toast.success("Status Updated", {duration: 1000});
        }
        else{
            console.log(123);
            toast.error("Message Required", {duration: 1000})
        }
                ...
                <div className="flex flex-wrap justify-center">
                    <div className="w-full text-center mt-4">
                        ...
                        <div className="w-full flex justify-center mt-8 mb-4">
                            <div>
                                <textarea id="message" rows={2} ref={inputRef} className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Your message..."></textarea>
                                ...
                                <button type="button" onClick={() => handleClick()} className="inline-block mt-6 px-6 py-2.5 bg-gray-400 text-gray-700 font-medium text-xs leading-tight uppercase rounded-full shadow-md hover:bg-gray-300 hover:shadow-lg focus:bg-gray-300 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-gray-400 active:shadow-lg transition duration-150 ease-in-out">
                                    Update
                                </button>
                            </div>
                        </div>
